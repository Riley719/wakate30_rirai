...
これを用いると$ini(S)$はとある$ZZ^3$の部分モノイドのモノイド環になっているが，計算結果から少なくとも生成系の一部として$(1,0,0), (1,1,0), (1,2,0), (1,3,0), (1,4,0), (2,3,1), (1,5,0), (1,6,0)$が取れるとわかる．これら元があれば，そのグロタンディーク群は$ZZ^3$全体になるので，定理21.4より$dim(ini(S)) = dim(k[x,y,z,x^(-1),y^(-1),z^(-1)]) = 3$が従う．主定理の適用結果は正しかったと分かる．
