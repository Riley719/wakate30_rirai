...
また，ノートやスライド，本報告書のPDFやTypstのソースコードは次のGitHubにおいてある。

#github-icon() #link("https://github.com/Riley719/wakate30_rirai")
...
ところで，可換代数において「局所環」と書いてある部分を「$H$局所環」と変えると，類似の定理が成立するという観察がある．詳しくは述べないが，クルルの次元定理も次のスモークの次元定理として類似の定理が成り立つ．次は@Smo72[Theorem 5.5] による．
...
イニシャル代数とSagbi基底については@RS90 を参照されたい．多項式を斉次としているのは，$S$が次数環になるようにするためだけに置いている仮定である．

与えられた部分代数からSagbi基底を計算する手法が確立しており，Macaulay2でもそれは実装されている．@BCD, @BCD24 しかし，Gr\u{00F6}bner基底と違って，有限なSagbi基底を持つとは限らない．
...
イニシャル代数をとると，このように構造が破壊される．しかし，次のようにポアンカレ級数は保存することが知られている．@CHV96[Proposition 2.4] 
...
ちなみに今回の実際のクルル次元はどうなっているかというと，それは$3$である．これを説明しよう．まず，イニシャル代数をとると必ず単項式代数になることはやさしい．イニシャル代数のクルル次元を観察するうえで次の定理は大変強力である．@AG76
...
また，与えられた$frak(p) in Spec(R)$に対して$frak(p)^* = (x in frak(p) | x "は斉次元")$と定めると，これも$R$の素イデアルとなる．これに関して次の定理がある．@MR74[Lemma 1] では近いことが述べられているが，この証明は$frak(p)$の高さに関する帰納法であり，ネーター性が仮定されているものと思われるので，改めて証明する．
...
次にGK次元を定義する．GK次元は非可換環やワイル代数に対する研究において導入されたと思われる。初出は@GK66 であろう．詳細な性質などは@KL85 を参照されたい．
...
次に，ヒルベルトの第14問題の反例として有名なNagataの例を考える．@Nag61
  ...
  #grid(
    ...
    ..range(1, 27).map(p => 
      rect(
        ...
        image("slide.pdf", page: p, width: 100%) // 枠に合わせて幅を指定
      )
    )
  )
]
